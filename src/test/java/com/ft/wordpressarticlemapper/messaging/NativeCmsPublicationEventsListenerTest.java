...
public class NativeCmsPublicationEventsListenerTest {
  ...
  private static final String MESSAGE_BODY_FOR_DELETE =
      "{\"error\":\"Not found.\",\"lastModified\":\"2016-12-13T08:45:35.151Z\",\"post\":{\"uuid\":\"8c4d6fea-1c49-33f7-5500-53dfc3335d88\"},\"status\":\"error\"}";

  private NativeCmsPublicationEventsListener listener;
  private NativeWordPressContent nativeWordPressContent;

  @Mock private MessageProducingContentMapper mapper;

  @Mock private ObjectMapper objectMapper;

  @Mock private ObjectReader objectReader;

  @Mock private NativeWordPressContentValidator contentValidator;
  ...
  public void setUp() throws Exception {
    listener =
        new NativeCmsPublicationEventsListener(mapper, objectMapper, SYSTEM_CODE, contentValidator);
    nativeWordPressContent = createSampleWordpressArticle();
    when(objectReader.readValue(loadFile("messaging/native-wordpress-content.json")))
        .thenReturn(nativeWordPressContent);
  ...
  public void thatMapForPublishIsCalledWhenStatusIsOk() throws Exception {
    ...
    verify(mapper, times(1))
        .mapForPublish(eq(TX_ID), c.capture(), eq(message.getMessageTimestamp()));
  ...
  public void thatMapForDeleteIsCalledWhenStatusIsError() throws ParseException, IOException {
    ...
    NativeWordPressContent expectedNativeContent =
        getNativeContentForDelete(uuid, messageTimestamp);
    doThrow(PostNotFoundException.class)
        .when(contentValidator)
        .validate(any(NativeWordPressContent.class));
