...
public class MessageProducingContentMapperTest {
  ...
  private static final UriBuilder URI_BUILDER =
      UriBuilder.fromUri("http://www.example.org/content").path("{uuid}");
  private static final String PUBLISH_REF = "tid_yoyfkwcs2s";
  private static final MessageType CMS_CONTENT_PUBLISHED =
      MessageType.messageType("cms-content-published");
  private static final String CANONICAL_WEB_URL_TEMPLATE = "https://www.ft.com/content/%s";

  @Mock private WordPressBlogPostContentMapper wordPressBlogPostContentMapper;
  @Mock private WordPressLiveBlogContentMapper wordPressLiveBlogContentMapper;
  ...
  public void setUp() throws Exception {
    NativeWordPressContent content =
        JACKSON_MAPPER
            .reader(NativeWordPressContent.class)
  ...
  public void testMessageForPublishIsProducedAndSent() throws Exception {
    ...
    WordPressBlogPostContent content =
        JACKSON_MAPPER
            .reader(WordPressBlogPostContent.class)
    ...
    when(wordPressBlogPostContentMapper.mapWordPressArticle(
            eq(PUBLISH_REF), eq(post), eq(lastModified)))
        .thenReturn(content);
  ...
  private void verifyMessage(Message actualMessage, WordPressContent expectedContent)
    ...
    assertThat(
        messageBody.get("contentUri"), equalTo("http://www.example.org/content/" + post.getUuid()));
    ...
    assertThat(
        actualContent.get("publishReference"), equalTo(expectedContent.getPublishReference()));
    assertThat(
        OffsetDateTime.parse((String) actualContent.get("lastModified")).toInstant(),
        equalTo(expectedContent.getLastModified().toInstant()));
  ...
  public void testThatMessageForDeleteIsProducedAndSent() throws IOException {
    ...
    assertThat(
        OffsetDateTime.parse((String) messageBody.get("lastModified")).toInstant(),
        equalTo(messageTimestamp.toInstant()));
  }
  ...
  public void testNoMessageIsSentWhenObjectMapperFails() throws Exception {
    ...
    when(failing.writeValueAsString(any()))
        .thenThrow(new JsonGenerationException("test exception"));
  ...
  public void testWordPressBlogContentMapperIsCalledForPostMessages() throws Exception {
    ...
    verify(wordPressBlogPostContentMapper, times(1))
        .mapWordPressArticle(PUBLISH_REF, post, lastModified);
  }

  @Test
  public void testWordPressLiveBlogContentMapperIsCalledForLiveMessages() throws Exception {
    Date lastModified = new Date();
    WordPressLiveBlogContent content =
        JACKSON_MAPPER
            .reader(WordPressLiveBlogContent.class)
            .readValue(loadFile("messaging/wordpress-live-blog-content-test.json"));
    post.setType("webchat-live-blogs");

    when(wordPressLiveBlogContentMapper.mapWordPressArticle(
            eq(PUBLISH_REF), eq(post), eq(lastModified)))
        .thenReturn(content);

    mapper.mapForPublish(PUBLISH_REF, post, lastModified);

    verify(wordPressLiveBlogContentMapper, times(1))
        .mapWordPressArticle(PUBLISH_REF, post, lastModified);
  }
