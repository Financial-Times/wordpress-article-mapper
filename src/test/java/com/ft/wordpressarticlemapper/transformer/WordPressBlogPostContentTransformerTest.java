...
public class WordPressBlogPostContentTransformerTest {
  ...
  private static final OffsetDateTime PUBLISHED_DATE =
      OffsetDateTime.parse("2015-09-30T15:30:00.000Z");
  private static final String PUBLISHED_DATE_STR =
      PUBLISHED_DATE.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
  private static final OffsetDateTime MODIFIED_DATE =
      OffsetDateTime.parse("2015-10-30T12:13:00.000Z");
  private static final String MODIFIED_DATE_STR =
      MODIFIED_DATE.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
  ...
  private static final SortedSet<Identifier> IDENTIFIERS =
      ImmutableSortedSet.of(new Identifier(SYSTEM_ID, POST_URL));
  ...
  private BodyProcessingFieldTransformer bodyTransformer =
      mock(BodyProcessingFieldTransformer.class);
  ...
  public void setUp() {
    ...
    when(identifierBuilder.buildIdentifiers(eq(requestUri), any(Post.class)))
  ...
  @Test
  public void thatBlogPostWithFeaturedImageIsTransformed() throws Exception {
  ...
  public void thatBlogPostContainingOnlyUnsupportedTagsIsRejected() {
    ...
    when(bodyTransformer.transform("<body>" + unsupportedBody + "</body>", TX_ID))
        .thenReturn("<body>\n\n</body>");
  ...
  public void thatEmptyPublishedDateIsCorrectlyHandled() {
    ...
    assertThat(
        "publishedDate",
        actual.getPublishedDate().toInstant(),
        is(equalTo(MODIFIED_DATE.toInstant())));
    assertThat("lastModified", actual.getLastModified(), is(equalTo(LAST_MODIFIED)));
