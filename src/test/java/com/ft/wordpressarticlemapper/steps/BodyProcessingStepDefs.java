...
public class BodyProcessingStepDefs {
  ...
  private static final ConvertParameters CONVERT_PARAMETERS =
      new ConvertParameters(CONVERT_FROM_PARAMETER, CONVERTED_TO_PARAMETER, CONVERSION_TEMPLATE);
  private static final List<ConvertParameters> CONVERT_PARAMETERS_LIST =
      ImmutableList.of(CONVERT_PARAMETERS);
  ...
  public static List<VideoSiteConfiguration> DEFAULTS =
      Arrays.asList(
          new VideoSiteConfiguration(
          ...
          new VideoSiteConfiguration(
              ...
              true),
          new VideoSiteConfiguration(
              "https?://www.vimeo.com/(?<id>[0-9]+)", null, false, NONE, null, true),
          new VideoSiteConfiguration(
              "//player.vimeo.com/video/(?<id>[0-9]+)",
              ...
              true),
          new VideoSiteConfiguration(
              "https?://video.ft.com/(?<id>[0-9]+)/", null, false, NONE, null, true));
  ...
  public void setup() {
    ...
    List<BlogApiEndpointMetadata> metadataList =
        ImmutableList.of(
            new BlogApiEndpointMetadata(
                fastFtHost, fastFtBrands, fastFtId, Syndication.YES.getCanBeSyndicated()));
    ...
    clientMockBuilder.mockResolverRedirect(
        resolverClient, URI.create("http://on.ft.com/1NVIQzo"), identifierValue);
    ...
    clientMockBuilder.mockContentRead(
        contentReadClient, CONTENT_READ_URI, CONTENT_UUID, CONTENT_READ_HOST_HEADER, SC_OK);
    ...
    rulesAndHandlers.put(
        "RETAIN ELEMENT AND REMOVE ATTRIBUTES", "RetainWithoutAttributesXMLEventHandler");
    rulesAndHandlers.put("TRANSFORM THE TAG", "SimpleTransformTagXmlEventHandler");
    rulesAndHandlers.put(
        "CONVERT HTML ENTITY TO UNICODE", "PlainTextHtmlEntityReferenceEventHandler");
  ...
  public void I_have_a_rule_and_an_entity(String rule, String entity) throws Throwable {
    ...
    assertThat(
        "The handler is incorrect", eventHandler.getClass().getSimpleName(), equalTo(handler));
  }
  ...
  private void assertTagIsRegisteredToTransform(String rule, String before, String after) {
    ...
    assertThat(
        "The replacement tag is not registered properly",
        eventHandler.getNewElement(),
        equalTo(after));
  }

  private XMLEventHandler assertTagIsRegistered(String name, String rule) {
    String handler = rulesAndHandlers.get(rule);
    StartElementEventImpl startElement =
        StartElementEventImpl.construct(null, new QName(name), null, null, null);
  ...
  public void the_fastFT_body_contains_transforms_into(
      String replacement, String tagname, String rule) throws Throwable {
