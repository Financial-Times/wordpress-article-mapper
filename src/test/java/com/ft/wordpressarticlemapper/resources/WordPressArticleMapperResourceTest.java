...
public class WordPressArticleMapperResourceTest {
  ...
  private static final Brand ALPHA_VILLE_BRAND =
      new Brand("http://api.ft.com/things/89d15f70-640d-11e4-9803-0800200c9a66");
  ...
  public void mapShouldUnescapeHtmlNumericalEntityForTitleAndByline() throws Exception {
    ...
    final String sourceApiJson =
        loadFile(
            "wordPress/__files/WILL_RETURN_200-body-no-html-entity-number-from-wordpress.json");
    ...
    assertThat(
        "byline",
        receivedContent.getByline(),
        is(equalTo("<FT Labs Administrator>, <Jan Majek>, <Adam Braimbridge>")));
  }
  ...
  public void mapShouldUnescapeHtmlNamedEntityForTitleAndByline() throws Exception {
    final URI uri = buildMapperUrl("map");
    final String sourceApiJson =
        loadFile("wordPress/__files/WILL_RETURN_200-body-no-html-entity-name-from-wordpress.json");
    ...
    assertThat(
        "title",
        receivedContent.getTitle(),
        is(equalTo("The £64 million pound question & what it means for the EU…")));
    assertThat(
        "byline",
        receivedContent.getByline(),
        is(equalTo("€FT Labs Administrator‰, £Jan Majek™, ¥Adam Braimbridge¾")));
  ...
  public void mapShouldReturn200AndCompleteResponseWhenContentFoundInWordPress() throws Exception {
    ...
    final URI uri = buildMapperUrl("map");
    final String sourceApiJson =
        loadFile("wordPress/__files/WILL_RETURN_200-body-from-wordpress.json");
    ...
    WordPressBlogPostContent receivedContent =
        clientResponse.getEntity(WordPressBlogPostContent.class);
    assertThat("title", receivedContent.getTitle(), is(equalTo("The 6am London Cut")));
    assertThat("type", receivedContent.getType(), is(equalTo(TYPE_ARTICLE)));
    assertThat(
        "body",
        receivedContent.getBody(),
    ...
    assertThat(
        "identifier",
        receivedContent.getIdentifiers(),
        hasItem(
            new Identifier(
                "http://api.ft.com/system/FT-LABS-WP-1-24",
                "http://uat.ftalphaville.ft.com/2014/10/21/2014692/the-6am-london-cut-277/")));
    assertThat(
        "identifier",
        receivedContent.getIdentifiers(),
        hasItem(
            new Identifier(
                "http://api.ft.com/system/FT-LABS-WP-1-24",
                "http://uat.ftalphaville.ft.com/?p=2014692")));
    assertThat(
        "uuid", receivedContent.getUuid(), is(equalTo("5c652c7e-c81e-4be7-8669-adeb5a5621dd")));
  ...
  public void mapShouldReturn422WhenTypeNotPostFromWordpressResponse() throws Exception {
    final URI uri = buildMapperUrl("map");
    final String sourceApiJson =
        loadFile("wordPress/__files/WILL_RETURN_200-incorrect-blog-type.json");
    ...
    assertThat(
        "response status", clientResponse, hasProperty("status", equalTo(SC_UNPROCESSABLE_ENTITY)));
    assertThat(
        "response message",
        clientResponse.getEntity(String.class),
        containsString("Wordpress content is not valid"));
  }
  ...
  public void mapShouldReturn422WhenPostContainsOnlyUnsupportedContent() throws Exception {
    final URI uri = buildMapperUrl("map");
    final String sourceApiJson =
        loadFile("wordPress/__files/WILL_RETURN_200-unsupported-content.json");
  ...
  public void mapShouldReturn422WhenApiUrlIsMissingFromWordpressResponse() throws Exception {
    final URI uri = buildMapperUrl("map");
    final String sourceApiJson =
        loadFile("wordPress/__files/WILL_RETURN_200-no-apiurl-on-response.json");
  ...
  @Test
  public void ingestShouldReturn422WhenPostContainsUnsupportedContentAndNoMessageShouldBeSent()
      throws Exception {
    final URI uri = buildMapperUrl("ingest");
    final String sourceApiJson =
        loadFile("wordPress/__files/WILL_RETURN_200-unsupported-content.json");
