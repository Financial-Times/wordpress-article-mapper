...
public class ClientMockBuilder {
  ...
  public void mockDocumentStoreQuery(
      Client documentStoreQueryClient, URI queryURI, URI to, int status) {
  ...
  public void mockDocumentStoreQuery(
      Client documentStoreQueryClient,
      URI documentStoreQueryBaseUri,
      URI authority,
      URI identifierValue,
      URI to,
      int status) {
    URI queryURI =
        buildDocumentStoreQueryUri(documentStoreQueryBaseUri, authority, identifierValue);
    mockDocumentStoreQuery(documentStoreQueryClient, queryURI, to, status);
  }

  public URI buildDocumentStoreQueryUri(
      URI documentStoreQueryUri, URI authority, URI identifierValue) {
    URI queryURI = null;
    try {
      queryURI =
          UriBuilder.fromUri(documentStoreQueryUri)
              .queryParam(
                  "identifierAuthority", URLEncoder.encode(authority.toASCIIString(), "UTF-8"))
              .queryParam(
                  "identifierValue", URLEncoder.encode(identifierValue.toASCIIString(), "UTF-8"))
              .build();
