...
public class CanConnectToMessageQueueProducerProxyHealthcheck extends AdvancedHealthCheck {

  private final QueueProxyHealthcheck queueProxyHealthcheck;
  private final HealthcheckConfiguration healthcheckConfiguration;
  private Timer timer;

  public CanConnectToMessageQueueProducerProxyHealthcheck(
      final QueueProxyHealthcheck queueProxyHealthcheck,
    ...
    if (metrics != null) {
      timer =
          metrics.timer(
              MetricRegistry.name(
                  CanConnectToMessageQueueProducerProxyHealthcheck.class, "checkAdvanced"));
    } else {
      timer = new Timer();
    }
  }
