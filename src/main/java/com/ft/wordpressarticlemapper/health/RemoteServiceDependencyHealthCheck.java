...
public class RemoteServiceDependencyHealthCheck extends AdvancedHealthCheck {
  ...
  private static final Logger LOGGER =
      LoggerFactory.getLogger(RemoteServiceDependencyHealthCheck.class);
  ...
  public RemoteServiceDependencyHealthCheck(
  ...
  protected AdvancedResult checkAdvanced() throws Exception {
    ...
    try {
      response = client.resource(pingUri).header("Host", remoteHost).get(ClientResponse.class);
  ...
  protected String technicalSummary() {
    return String.format(
        "Tests that the /__health endpoint for the %s returns 200 HTTP status response",
        remoteServiceName);
  }
}
