...
public abstract class WordPressContentMapper<C extends WordPressContent> {
  ...
  protected abstract C doMapping(
  ...
  private Date extractFirstPublishedDate(URI requestUri, Post post) {
    ...
    if (firstPublishedDateStr == null) {
      LOG.info(
          "First published date cannot be determined as date GMT field is empty. Request URI: "
              + requestUri);
      return null;
    }
    return Date.from(
        OffsetDateTime.parse(firstPublishedDateStr + "Z", PUBLISH_DATE_FMT).toInstant());
  }
