...
public class WordPressBlogPostContentMapper
    extends WordPressContentMapper<WordPressBlogPostContent> {
  ...
  public WordPressBlogPostContentMapper(
  ...
  @Override
  protected WordPressBlogPostContent doMapping(
    ...
    if (Strings.isNullOrEmpty(body)) {
      throw new UnpublishablePostException(
          uuid.toString(), "Not a valid WordPress article for publication - body of post is empty");
    }
    body = wrapBody(body);

    WordPressBlogPostContent.Builder builder =
        (WordPressBlogPostContent.Builder)
            WordPressBlogPostContent.builder()
                .withUuid(uuid)
    ...
    if (Strings.isNullOrEmpty(unwrapBody(transformedBody))) {
      throw new UntransformablePostException(
          uuid.toString(),
  ...
  private String unwrapBody(String wrappedBody) {
    ...
    return wrappedBody
        .substring(START_BODY.length(), wrappedBody.length() - END_BODY.length())
        .trim();
  }
}
