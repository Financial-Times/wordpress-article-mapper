...
public class WordPressArticleMapperResource {
  ...
  public WordPressArticleMapperResource(
      WordPressBlogPostContentMapper blogTransformer,
  ...
  public final WordPressContent map(
      NativeWordPressContent nativeWordPressContent, @Context HttpHeaders httpHeaders) {
    ...
    try {
      ...
      return transformerFor(postDetails)
          .mapWordPressArticle(transactionId, postDetails, new Date());
      ...
      throw new ClientError.ClientErrorBuilder(SC_UNPROCESSABLE_ENTITY)
          .error("Wordpress content is not valid")
          .exception(e);
    }
  }
  ...
  private WordPressContentMapper<?> transformerFor(Post post) {
    ...
    try {
      ...
      /* ignore and throw as below */
    }

    if (wordPressPostType == null) {
      throw new ServerError.ServerErrorBuilder(SC_UNPROCESSABLE_ENTITY)
          .error("unsupported blog post type")
          .exception();
    }
