...
public class IdentifierBuilder {
  ...
  public SortedSet<Identifier> buildIdentifiers(URI requestUri, Post post) {
    ...
    String additionalIdentifierValue =
        buildWordpressAdditionalIdentifier(blogApiEndpointMetadata, post);
    identifiers.add(new Identifier(originatingSystemId, additionalIdentifierValue));

    return identifiers;
  }

  private String buildWordpressAdditionalIdentifier(
      BlogApiEndpointMetadata blogApiEndpointMetadata, Post post) {
    ...
    URI wordpressAlternativeUri =
        UriBuilder.fromPath(path).host(host).scheme(scheme).queryParam("p", post.getId()).build();
