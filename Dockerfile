...
CMD echo wordpress.contentApi.key=$WORDPRESS_CONTENTAPI_KEY > /credentials.properties
CMD java -Ddw.credentialsPath=/credentials.properties -Ddw.healthCheckWordPressConnections[0].hostName=ftalphaville.ft.com -Ddw.server.applicationConnectors[0].port=8080 -Ddw.server.adminConnectors[0].port=8081 -jar app.jar server config.yaml
