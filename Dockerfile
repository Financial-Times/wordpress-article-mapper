...
RUN apk --update add git \
 ...
 && mv target/wordpress-article-transformer-*.jar /wordpress-article-transformer.jar \
...
CMD exec java $JAVA_OPTS \
     -Ddw.nativeReaderConfiguration.endpointConfiguration.primaryNodes=$READ_ENDPOINT \
     ...
     -jar wordpress-article-transformer.jar server config.yaml
